...
def cardSearch():
	...
	advanced_search_form = advanced_card_search()
	attack_text = advanced_search_form["attack_text"]
	if(attack_text != ""):
		cards = getCards_attackText(attack_text)
		cards = sorted(cards, key = lambda x: x[0])
	# elif(ability_text != ""):
	# 	cards = getCards_abilityText(ability_text)
	# 	cards = sorted(cards, key = lambda x: x[0])
	else:
		cards = getAllCards()
	return render_template("proxies.html", allCards = cards)
