...
def getAllCards():
	...
	conn = sqlite3.connect("pokemontcg.db")
	cur = conn.cursor()
	query = "SELECT name, card_set, set_num, api_card_id, imageURL FROM Card WHERE card_set = ?"
...
def getCards_attackText(text):
	# @ desc selects cards from the database that have an attack containing text
	# @ param the text that the user would like all cards to contain
	# @ return a list of the cards that have that attack text

	conn = sqlite3.connect("pokemontcg.db")
	cur = conn.cursor()

	# query = "SELECT attack_id FROM Attacks instr(attack_text, ?) > 0"
	# cur.execute(query, (text,))

	# attack_ids = cur.fetchall()

	# query = " "


	query = "SELECT name, card_set, card_id, api_card_id, imageURL FROM Card WHERE card_id IN (SELECT card_id FROM hasAttack WHERE attack_id IN (SELECT attack_id FROM Attacks WHERE attack_text LIKE ?"
	cur.execute(query, ("%" + str(text) + "%",))
	results = cur.fetchall()
